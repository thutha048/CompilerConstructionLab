...
void compileStatements2(void) {
  // TODO ???
  while (lookAhead->tokenType == SB_SEMICOLON ||lookAhead->tokenType == SB_PLUS||lookAhead->tokenType == SB_MINUS||lookAhead->tokenType == SB_TIMES
...
void compileAssignSt(void) {
  ...
  case SB_ASSIGN_SUBTRACT:
      ...
      compileExpression();
      assert("Assign SUBTRACT statement parsed ....");
      break;
  case SB_ASSIGN_TIMES:
      eat(SB_ASSIGN_TIMES);
      compileExpression();
      assert("Assign TIMES statement parsed ....");
      break;
 case SB_ASSIGN_DIVIDE:
      eat(SB_ASSIGN_DIVIDE);
      compileExpression();
      assert("Assign DIVIDE statement parsed ....");
      break;
  default:
    error(ERR_INVALIDSTATEMENT, lookAhead->lineNo, lookAhead->colNo);
    assert("Assign statement parsed ....");
...
void compileTerm2(void) {
  ...
  if(lookAhead->tokenType == SB_TIMES || lookAhead->tokenType == SB_SLASH ||lookAhead->tokenType == SB_MODULE){
  switch (lookAhead->tokenType) {
  case SB_TIMES:
    eat(SB_TIMES);
    compileFactor();
